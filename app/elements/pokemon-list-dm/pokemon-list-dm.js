...
class PokemonListDm extends LitElement {
  ...
  getResults(results) {
    ...
    dp.addEventListener("request-success", (ev) => {
      data.push({
        name: ev.detail.chain.species.name,
        id: this.setID(ev.detail.chain.species.url),
        evolutions: ev.detail.chain.evolves_to,
      });
  ...
  render() {
    return html` <bbva-core-generic-dp id="results"></bbva-core-generic-dp> `;
  }
}
